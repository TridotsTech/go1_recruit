...
def get_context(context):
	...
	context.token=Token
	encrypt_url=frappe.db.sql('''select exam_result_id,email from `tabExam Result Sharing` where encrypted_url=%(token)s''',{'token':'{0}/result?token={1}'.format(frappe.utils.get_url(), Token)},as_dict=1)
	if encrypt_url:
